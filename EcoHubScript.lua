...
Tabs.Main:AddInput("DistanceInput", {
    ...
    if value then
        ...
        task.spawn(function()
            while _G.AutoFarm do
                pcall(function()
                    for _, mob in pairs(workspace.Mob:GetChildren()) do
                        if mob.Name == selectedMob and mob:FindFirstChild("Humanoid") and mob.Humanoid.Health > 0 then
...
for key, id in pairs(skillKeys) do
    Tabs.Settings:AddToggle(id, {
        ...
        Callback = function(state)
            if state then
                task.spawn(function()
                    while Tabs.Settings.Toggles[id].Value do
